...
class T2T_module(nn.Module):
    ...
    def __init__(self, img_size=224, tokens_type='performer', in_chans=3, embed_dim=768, token_dim=64):
        ...
        if tokens_type == 'transformer':
            ...
            self.soft_split0 = nn.Conv2d(3, token_dim, kernel_size=(3, 3), stride=(4, 4), padding=(2, 2))  # the 1st convolution
    ...
    def forward(self, x):
        # step0: soft split
        x1 = self.soft_split0(x).transpose(1, 2)

        # iteration1: re-structurization/reconstruction
        x2 = self.attention1(x1)
        B, new_HW, C = x2.shape
        x3 = x2.transpose(1,2).reshape(B, C, int(np.sqrt(new_HW)), int(np.sqrt(new_HW)))
        # iteration1: soft split
        x4 = self.soft_split1(x3).transpose(1, 2)

        # iteration2: re-structurization/reconstruction
        x5 = self.attention2(x4)
        B, new_HW, C = x5.shape
        x6 = x5.transpose(1, 2).reshape(B, C, int(np.sqrt(new_HW)), int(np.sqrt(new_HW)))
        # iteration2: soft split
        x7 = self.soft_split2(x6).transpose(1, 2)
